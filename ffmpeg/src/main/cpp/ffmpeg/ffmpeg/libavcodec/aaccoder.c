    ...
    for (swb = 0; swb < max_sfb; swb++)
    {
        ...
        else
        {
            ...
            for (cb = 0; cb < CB_TOT_ALL; cb++)
            {
                ...
                if (run_value_bits[sce->ics.num_windows == 8][path[swb][cb].run] != run_value_bits[sce->ics.num_windows == 8][path[swb][cb].run + 1])
        ...
        for (g = 0; g < sce->ics.num_swb; g++)
        {
            ...
            if (nz)
            {
                ...
                for (q = minscale; q < maxscale; q++)
                {
                    ...
                    {
                        float cost;
                        cost = paths[idx - 1][i].cost + dist + ff_aac_scalefactor_bits[q - i + SCALE_DIFF_ZERO];
...
static void search_for_pns(AACEncContext *s, AVCodecContext *avctx, SingleChannelElement *sce)
{
    ...
    int refbits = avctx->bit_rate * 1024.0 / avctx->sample_rate / ((avctx->flags & AV_CODEC_FLAG_QSCALE) ? 2.0f : avctx->channels) * (lambda / 120.f);
        ...
        for (g = 0; g < sce0->ics.num_swb; g++)
        {
            ...
            if (!sce0->zeroes[w * 16 + g] && !sce1->zeroes[w * 16 + g] && !cpe->is_mask[w * 16 + g])
            {
                ...
                for (w2 = 0; w2 < sce0->ics.group_len[w]; w2++)
                {
                    ...
                    {
                        M[i] = (sce0->coeffs[start + (w + w2) * 128 + i] + sce1->coeffs[start + (w + w2) * 128 + i]) * 0.5;
                ...
                for (sid_sf_boost = 0; sid_sf_boost < 4; sid_sf_boost++)
                {
                    ...
                    if (sce0->band_type[w * 16 + g] != NOISE_BT && sce1->band_type[w * 16 + g] != NOISE_BT && (!ff_sfdelta_can_replace(sce0, nextband0, prev_mid, mididx, w * 16 + g) || !ff_sfdelta_can_replace(sce1, nextband1, prev_side, sididx, w * 16 + g)))
                    {
