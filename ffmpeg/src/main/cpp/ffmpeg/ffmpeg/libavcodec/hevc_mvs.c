                                     ...
                                     int nPbW, int nPbH)
{
    ...
    lc->na.cand_up_right_sap =
        ((x0b + nPbW) == (1 << s->ps.sps->log2_ctb_size)) ? lc->ctb_up_right_flag && !y0b : lc->na.cand_up;
    lc->na.cand_up_right =
        lc->na.cand_up_right_sap && (x0 + nPbW) < lc->end_of_tiles_x;
                                            ...
                                            struct MvField mergecandlist[])
{
    ...
    const int nb_refs = (s->sh.slice_type == HEVC_SLICE_P) ? s->sh.nb_refs[0] : FFMIN(s->sh.nb_refs[0], s->sh.nb_refs[1]);
        ...
        nb_merge_cand < s->sh.max_num_merge_cand)
    {
        ...
        int available_l0 = temporal_luma_motion_vector(s, x0, y0, nPbW, nPbH,
                                                       0, &mv_l0_col, 0);
        int available_l1 = (s->sh.slice_type == HEVC_SLICE_B) ? temporal_luma_motion_vector(s, x0, y0, nPbW, nPbH,
                                                                                            0, &mv_l1_col, 1)
                                                              : 0;
        ...
        nb_orig_merge_cand < s->sh.max_num_merge_cand)
    {
        ...
        for (comb_idx = 0; nb_merge_cand < s->sh.max_num_merge_cand &&
                           comb_idx < nb_orig_merge_cand * (nb_orig_merge_cand - 1);
             comb_idx++)
        {
